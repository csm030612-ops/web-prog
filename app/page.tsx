...
export default async function HomePage({
  ...
  const isFiltered = !!(params.category || params.type || params.q)
    ...
    <div>
      ...
      {!user && (
        <section style={{
          ...
          <div style={{ position: 'relative', maxWidth: 600, margin: '0 auto' }}>
            ...
            <p style={{ fontSize: 17, opacity: 0.85, marginBottom: 36, lineHeight: 1.6 }}>
      ...
      {user && (
        <section style={{
          ...
          <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
            <div>
              <p style={{ fontSize: 13, opacity: 0.75, marginBottom: 2 }}>오늘도 함께해요 👋</p>
              <p style={{ fontSize: 20, fontWeight: 800, letterSpacing: '-0.02em' }}>
                어떤 재능을 찾고 계세요?
              </p>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <Link href="/listings/new" className="btn" style={{
                background: 'var(--accent)', color: 'white',
                padding: '10px 20px', fontSize: 13, borderRadius: 10,
              }}>
                + 재능 등록
              </Link>
              <Link href="/board/new" className="btn" style={{
                background: 'rgba(255,255,255,0.15)', color: 'white',
      ...
      {!user && (
        <section style={{ background: 'white', borderBottom: '1.5px solid var(--border)' }}>
          <div className="container" style={{ display: 'flex', justifyContent: 'center', gap: 48, padding: '18px 24px' }}>
            {[
      ...
      <section className="container" style={{ padding: '28px 24px 0' }}>
        {/* 검색창 */}
        <Suspense fallback={
          <div style={{ height: 48, borderRadius: 12, background: 'var(--surface-2)', marginBottom: 14 }} />
        }>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
          {[
            { label: '전체', value: '' },
            { label: '🎁 제공', value: 'offer' },
            { label: '🙋 요청', value: 'request' },
          ].map(tab => {
            const active = (params.type ?? '') === tab.value
            const href = tab.value
              ? `/?type=${tab.value}${params.category ? `&category=${params.category}` : ''}${params.q ? `&q=${params.q}` : ''}`
              : `/?${params.category ? `category=${params.category}&` : ''}${params.q ? `q=${params.q}` : ''}`
            return (
        ...
        <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 8 }}>
          <Link href={`/?${params.type ? `type=${params.type}&` : ''}${params.q ? `q=${params.q}` : ''}`} style={{
            padding: '7px 14px', borderRadius: 100, fontSize: 12, fontWeight: 600,
            background: !params.category ? 'var(--primary)' : 'transparent',
          ...
          {CATEGORIES.map(cat => {
            ...
            const href = `/?category=${cat}${params.type ? `&type=${params.type}` : ''}${params.q ? `&q=${params.q}` : ''}`
            return (
      ...
      {/* ── 결과 헤더 ── */}
      <section className="container" style={{ padding: '16px 24px 0' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
            {isFiltered
              ? <><strong style={{ color: 'var(--text-primary)' }}>{listings?.length ?? 0}개</strong>의 결과</>
              : <><strong style={{ color: 'var(--text-primary)' }}>{listings?.length ?? 0}개</strong>의 재능</>
            }
            {params.q && <> — &quot;<strong>{params.q}</strong>&quot; 검색 결과</>}
          </p>
          {isFiltered && (
            <Link href="/" style={{ fontSize: 13, color: 'var(--primary)', fontWeight: 600 }}>
              필터 초기화
            </Link>
          )}
        </div>
      ...
      {/* ── 리스팅 그리드 ── */}
      <section id="listings" className="container" style={{ padding: '16px 24px 64px' }}>
        <div style={{
          display: 'grid',
          ...
          {(!listings || listings.length === 0) && (
            <div style={{
              ...
              <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 8 }}>
                {isFiltered ? '검색 결과가 없어요' : '아직 등록된 재능이 없어요'}
              </div>
              <p style={{ fontSize: 14 }}>
                {isFiltered ? '다른 키워드나 카테고리로 찾아보세요' : '첫 번째 재능을 등록해보세요!'}
              </p>
              {isFiltered
                ? <Link href="/" className="btn btn-outline" style={{ marginTop: 20 }}>전체 보기</Link>
                : <Link href="/listings/new" className="btn btn-primary" style={{ marginTop: 20 }}>재능 등록하기</Link>
              }
            </div>
          )}
        </div>
        ...
        <section style={{ background: 'white', padding: '64px 24px', borderTop: '1.5px solid var(--border)' }}>
          <div className="container">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24 }}>
              {[
                { icon: '🎁', title: '가입하면 100 크레딧',      desc: '바로 사용 가능한 100 크레딧을 드려요.' },
                { icon: '🔄', title: '재능 교환 (크레딧 무료)', desc: '서로 필요한 재능이 일치하면 크레딧 없이 교환!' },
                { icon: '💎', title: '크레딧으로 도움받기',     desc: '50 크레딧으로 도움받고, 제공하면 50 크레딧 획득.' },
                { icon: '⭐', title: '후기와 추천',             desc: '교환 후 서로 평가해 신뢰 커뮤니티를 만들어요.' },
              ].map((item, i) => (
                <div key={i} style={{
...
function ListingCard({ listing }: { listing: any }) {
  ...
  const avatarUrl = listing.profiles?.avatar_url
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 22px' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
            <span style={{ fontSize: 11, padding: '3px 8px', borderRadius: 6, background: 'var(--surface-2)', color: 'var(--text-secondary)', fontWeight: 600 }}>
              {listing.category}
            </span>
            {listing.tags?.slice(0, 2).map((tag: string) => (
              <span key={tag} style={{ fontSize: 11, padding: '3px 8px', borderRadius: 6, background: 'var(--surface-2)', color: 'var(--text-muted)' }}>
                #{tag}
              </span>
            ))}
          </div>
